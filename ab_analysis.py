...
parser.add_argument('-i', '--in', dest='input', required=True,
                    help="The input file, to be split and processed in parallel."
                         " If a directory is given, all files in the directory "
                         "will be iteratively processed.")
...
parser.add_argument('-l', '--log', dest='log', default='',
                    help="The log file, to which the BlastParse log info will be written. Default is stdout.")
...
parser.add_argument('-v', '--tsv', dest="tsv_out", action='store_true', default=False,
                    help="NOT YET IMPLEMENTED. If set, the final output"
                         "(from BlastParse) will be in tab-delimited format."
...
parser.add_argument('-m', '--merge', dest="merge", action='store_true', default=False,
                    help="Use if the input files are paired-end FASTQs "
                         ...
                         "Prior to running IgBLAST, reads will be merged with pandaseq."
...
parser.add_argument('-b', '--basespace', dest="use_basespace", default=False, action='store_true',
                    help="NOT YET IMPLEMENTED. Use flag if files should be downloaded directly from BaseSpace."
                         "Files will be downloaded into the directory provided with the '-i' flag, "
                         "which should be empty.")
parser.add_argument('-d', '--debug', dest="debug", action='store_true', default=False,
                    help="If set, will write all failed/exception sequences to file "
                         "and give more informative errors.")
parser.add_argument('-s', '--species', dest='species', default='human', choices=['human', 'macaque', 'mouse'])
...
def clean_up(temp_files, temp_out_files, temp_directory, temp_out_directory):
    for file_name in temp_out_files:
        os.remove(file_name)
...
def file_splitter(file_name, splitlen, num_seqs, temp_directory):
    counter = 1
    files_list = []
...
def check_input(input_list):
    current_format = format_check(input_list[0])
    if current_format == 'fasta':
        return input_list
    else:
