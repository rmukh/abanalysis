...
class BlastParse(object):
    ...
    def _j_indel_check(self):
        ...
        if '-' in self.join_nt_alignment:
            return True
        if '-' in self.join_nt_seq:
            return True
        return False
    ...
    def _var_region_nt_muts(self, r):
        ...
        for m in self.var_muts_nt:
            p = int(m.split(':')[0])
            if p < r['FWR1']['end'] + o:
                self.FR1_nt_muts.append(m)
            elif p < r['CDR1']['end'] + o:
                self.CDR1_nt_muts.append(m)
            elif p < r['FWR2']['end'] + o:
                self.FR2_nt_muts.append(m)
            elif p < r['CDR2']['end'] + o:
                self.CDR2_nt_muts.append(m)
            elif p < r['FWR3']['end'] + o:
                self.FR3_nt_muts.append(m)
    ...
    def _var_region_aa_muts(self, r):
        ...
        for m in self.var_muts_aa:
            p = int(m.split(':')[0])
            if p < (r['FWR1']['end'] + o) / 3:
                self.FR1_aa_muts.append(m)
            elif p < (r['CDR1']['end'] + o) / 3:
                self.CDR1_aa_muts.append(m)
            elif p < (r['FWR2']['end'] + o) / 3:
                self.FR2_aa_muts.append(m)
            elif p < (r['CDR2']['end'] + o) / 3:
                self.CDR2_aa_muts.append(m)
            elif p < (r['FWR3']['end'] + o) / 3:
                self.FR3_aa_muts.append(m)
        ...
        if self.chain == 'heavy':
            if not self.tsv:
                output = collections.OrderedDict([
                    ...
                    ('var_muts_aa', {'num': len(self.var_muts_aa),
                                     'muts': [{'loc': int(v.split(":")[0]), 'mut': v.split(":")[1]} for v in
                                              self.var_muts_aa]}),
                    ('v_ins',
                     [{'loc': int(i.split(",")[0]), 'len': int(i.split(",")[1]), 'seq': i.split(",")[2]} for i in
                      self.var_ins]),
                    ('v_del',
                     [{'loc': int(d.split(",")[0]), 'len': int(d.split(",")[1]), 'seq': d.split(",")[2]} for d in
                      self.var_del]),
                    ('d_ins',
                     [{'loc': int(i.split(",")[0]), 'len': int(i.split(",")[1]), 'seq': i.split(",")[2]} for i in
                      self.div_ins]),
                    ...
                    ('j_del',
                     [{'loc': int(d.split(",")[0]), 'len': int(d.split(",")[1]), 'seq': d.split(",")[2]} for d in
                      self.join_del]),
                    ('v_region_muts_nt', {'fr1': {'num': int(self.FR1_nt_muts_count),
                                                  'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                           in self.FR1_nt_muts]},
                                          'cdr1': {'num': int(self.CDR1_nt_muts_count),
                                                   'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                            in self.CDR1_nt_muts]},
                                          'fr2': {'num': int(self.FR2_nt_muts_count),
                                                  'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                           in self.FR2_nt_muts]},
                                          'cdr2': {'num': int(self.CDR2_nt_muts_count),
                                                   'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                            in self.CDR2_nt_muts]},
                                          'fr3': {'num': int(self.FR3_nt_muts_count),
                                                  'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                           in self.FR3_nt_muts]}}),
                    ('v_region_muts_aa', {'fr1': {'num': int(self.FR1_aa_muts_count),
                                                  'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                           in self.FR1_aa_muts]},
                                          'cdr1': {'num': int(self.CDR1_aa_muts_count),
                                                   'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                            in self.CDR1_aa_muts]},
                                          'fr2': {'num': int(self.FR2_aa_muts_count),
                                                  'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                           in self.FR2_aa_muts]},
                                          'cdr2': {'num': int(self.CDR2_aa_muts_count),
                                                   'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                            in self.CDR2_aa_muts]},
                                          'fr3': {'num': int(self.FR3_aa_muts_count),
                                                  'muts': [{'loc': int(m.split(":")[0]), 'mut': m.split(":")[1]} for m
                                                           in self.FR3_aa_muts]}}),
